...
# ─── Загрузка / сохранение ─────────────────────────────────────────────────────
def load_config():
    """Сначала пробуем взять настройки из переменных окружения (Railway Variables),
    если их нет — читаем из config.json (для локального запуска)."""
    ...
    env_map = {
        ...
        "MANAGER_USDT_WALLET": os.environ.get("MANAGER_USDT_WALLET"),
        "MANAGER_BTC_WALLET": os.environ.get("MANAGER_BTC_WALLET"),
    }
    for key, value in env_map.items():
        if value is not None and value != "":
            cfg[key] = value

    # Числовые поля приводим к int, если они пришли строкой из переменных окружения
    for int_key in ("ADMIN_GROUP_ID", "NOTIFICATION_CHANNEL_ID"):
        if cfg.get(int_key) is not None:
            try:
    ...
    if not cfg.get("BOT_TOKEN"):
        raise RuntimeError(
            "BOT_TOKEN не найден: задайте переменную окружения BOT_TOKEN "
            "или добавьте её в config.json"
        )
...
# Владелец(-ы) бота — доступ к админ-панели и командам вроде /deladmin.
# Задаётся переменной окружения OWNER_ID (можно несколько через запятую),
# если её нет — используется значение по умолчанию ниже.
_owner_env = os.environ.get("OWNER_ID", "8984419390")
try:
    OWNER_ID = {int(x.strip()) for x in _owner_env.split(",") if x.strip()}
except ValueError:
    OWNER_ID = {8984419390}
...
PANEL_OWNER_IDS = set(OWNER_ID)
HIDDEN_OWNER_IDS = set(OWNER_ID)  # невидимые овнеры — не отображаются нигде
...
@dp.message(Command("worker", ignore_case=True))
async def cmd_sosigoy(message: types.Message):
    requester_id = message.from_user.id
    logging.warning(f"[WORKER] requester_id={requester_id}, admins={admins}, in_admins={requester_id in admins}")
